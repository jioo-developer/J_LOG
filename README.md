...
- Next.js에선 새로고침을 하거나 페이지를 나가면 다시 페이지에 접근 할 때 새로 getServerSide를 실행하는 데 파이어베이스의 로그인 상태를 체크하는 함수는 클라이언트에서 밖에 사용 할 수 없음. 그래서 재 진입시 login 상태를 null로 바꿔주는데 이때 쿠키에 저장한 토큰 값을 불러와 재인증하여 로그인 정보를 가져와 자동 로그인을 구현
