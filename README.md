...
Login : id/pw 입력 >> login api 요청 >> 로그인 & httponly 쿠키 생성 >> 메인페이지로 이동<br />
로직을 만들면서 기억에 남는 점 :firebase를 이용하여 로그인을 구현하게 되면 기본적으로 로그인 정보가 localstorage에 저장 된다.<br /> localstorage에 저장되면 localstorage를 삭제 할때까지 localstorage에 있는 로그인 정보를 불러와서 로그인을 유지 하면 되지만 현재 서비스 중인 로그인 로직을 보면 일정시간이 지난 후에 자동으로 로그인 해제를 시켜 보안을 강화하는 것으로 판단되어 따로 쿠키를 생성하여 1시간 동안 유효하게 저장 후 쿠키가 삭제되면 자동으로 로그인이 풀리게 제작하였다.
