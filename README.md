...
- next.js 랜더링은 서버에서 api요청을 하여 데이터를 가져 온 후 완성된 html과 react에 필요한 js번들을 브라우저에 내리는 과정을 하는데 이때 서버에서 데이터를 가져 오는 과정에서 이미 로그인된 정보를 가져올 수 있는 방법이 없음. (로그인 시 쿠키를 저장한 이유)
