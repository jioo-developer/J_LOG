...
- 사실 좋아요 선택/취소 토글 만들기는 쉬웠는데 게시글당 1개의 좋아요를 어떻게 인식해서 페이지 정보에 가져 올 지 고민함
- firebase는 페이지(document)에 데이터를 남길수 있고 document 안에 또 db를 만들 수 있음<br /> 원래 여기 페이지에 해당하는 reply이란 DB를 만들었었는데 좋아요 DB를 추가해서 사용자의 uid를 남기고 페이지에 접속했을 때<br />그것을 불러와 유저의 uid와 비교해서 존재하면 좋아요 버튼을 disable 처리 함
